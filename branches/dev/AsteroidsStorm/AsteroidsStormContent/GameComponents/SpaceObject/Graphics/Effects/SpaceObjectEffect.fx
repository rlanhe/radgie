...
VertexShaderOutput BasicVS(
	 ...
	 float3 axis : BLENDINDICES )
{
	 float4x4 instanceWorld = mul(world, transpose(instanceTransform));
	 ...
	 // Rotate space object
	 float angle = time*rotationConst;
	 float cosValue = cos(angle);
	 float sinValue = sin(angle); 
	 	 
	 float3x3 u;
	 u[0] = float3(0,-axis.z,axis.y);
	 u[1] = float3(axis.z,0,-axis.x);
	 u[2] = float3(-axis.y,axis.x,0);

	 float3x3 uxu;
	 uxu[0] = float3(pow(axis.x,2),axis.x*axis.y,axis.x*axis.z);
	 uxu[1] = float3(axis.y*axis.x,pow(axis.y,2),axis.y*axis.z);
	 uxu[2] = float3(axis.z*axis.x,axis.z*axis.y,pow(axis.z,2));

	 float3x3 rotMatrix = mul(identity, cosValue) + mul(sinValue, u) + mul((1 - cosValue),uxu);
	 instanceWorld[0][0] = rotMatrix[0][0];
	 instanceWorld[0][1] = rotMatrix[0][1];
	 instanceWorld[0][2] = rotMatrix[0][2];
	 instanceWorld[1][0] = rotMatrix[1][0];
	 instanceWorld[1][1] = rotMatrix[1][1];
	 instanceWorld[1][2] = rotMatrix[1][2];
     instanceWorld[2][0] = rotMatrix[2][0];
     instanceWorld[2][1] = rotMatrix[2][1];
	 instanceWorld[2][2] = rotMatrix[2][2];

     //generate the world-view-projection matrix
     float4x4 wvp = mul(mul(instanceWorld, view), projection);
	 ...
	 worldToTangentSpace[0] = mul(tangent,instanceWorld);
	 worldToTangentSpace[1] = mul(cross(tangent,normal),instanceWorld);
	 worldToTangentSpace[2] = mul(normal,instanceWorld);
